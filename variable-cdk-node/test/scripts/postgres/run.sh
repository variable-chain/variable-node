...
main(){
    for origin_script in ${SCRIPT_FILES}; do
        ...
        docker cp "${script_file_path}" 37.27.27.226:"${script_file_path}"
        docker exec 37.27.27.226 bash -c "chmod a+x ${script_file_path} && psql ${DBNAME} ${DBUSER} -v ON_ERROR_STOP=ON --single-transaction -f ${script_file_path}"
